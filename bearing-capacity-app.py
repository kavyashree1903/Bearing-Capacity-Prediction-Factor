...
regressor = st.sidebar.selectbox("Select Regressor: ",("Linear Regression","KNN","SVM","Decision Trees",
                                                              "Random Forest","XGBoost"))

def input_variable():
    x1 = st.sidebar.slider('GSI', float(frprccolumns.x1.min()), float(frprccolumns.x1.max()),
    ...
    x3 = st.sidebar.slider('σci/γB', float(frprccolumns.x3.min()), float(frprccolumns.x3.max()),
                            float(frprccolumns.x3.mean()))
    x4 = st.sidebar.slider('d/B', float(frprccolumns.x4.min()),
                              float(frprccolumns.x4.max()), float(frprccolumns.x4.mean()))
    x5 = st.sidebar.slider('kh', float(frprccolumns.x5.min()),
